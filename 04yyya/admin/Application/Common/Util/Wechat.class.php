...
class Wechat
{
	...
	public function sendPayInfo($content, $openid, $sn, $price, $end_time='', $app = 0){
		$app_id = $this->appid;
		...
		if(!$app){
			$data = [
				...
				'nonce_str' => $NonceStr,
				'body' => $content,
				'notify_url' => 'http://api.'.WEB_DOMAIN.'/Order/Pay/wx_notify.html?paycode=' . C('payCode'),
				'openid' => $openid,
				'out_trade_no' => $sn,
				'total_fee' => $price,
				'time_expire' => date('YmdHis', $end_time),
				'trade_type' => 'JSAPI',
				'device_info' => 'WEB'
			];
		}else{
		...
		$sign = strtoupper(md5(join('&', $signArr) . "&key=" . C('WX_CONF.key')));
		$xml .= "<sign>{$sign}</sign>";
		$xml .= "</xml>";
		...
		if($xml['return_code'] == 'SUCCESS' && $xml['result_code'] == 'SUCCESS'){
			...
			$data['sign'] = strtoupper(md5(join('&', $signArr) . "&key=" . C('WX_CONF.key')));
			if(!$app)
				return $data;
			else{
				return [
