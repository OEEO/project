...
class Wechat
{
	...
	public function sendPayInfo($content, $openid, $sn, $price, $timeout='', $app = 0){
		$app_id = $this->getWXCONF('appid');
		$ip = $_SERVER['REMOTE_ADDR'];
		$mch_id = $this->getWXCONF('mchid');
		...
		if(!$app){
			$data = [
				...
				'nonce_str' => $NonceStr,
				'body' => $content,
				'notify_url' => 'http://api.'.DOMAIN.'/Order/Pay/wx_notify.html?paycode=' . C('payCode'),
				'openid' => $openid,
				'out_trade_no' => $sn,
				'total_fee' => $price,
				'trade_type' => 'JSAPI',
...
/*		if ($app == 0) {
		...
		*/
		if($timeout > time() && !empty($timeout)){
			$data['time_expire'] = date('YmdHis', $timeout);
		}
		//排序
		ksort($data);
		$signArr = [];
		...
		/*
		if ($app == 4) {
			$sign = strtoupper(md5(join('&', $signArr) . "&key=1WXUov4jDR9ZXHkZGxzjUuK673Aa9lA0"));
		} else {
			$sign = strtoupper(md5(join('&', $signArr) . "&key=" . $this->getWXCONF('key')));
		}*/
		$sign = strtoupper(md5(join('&', $signArr) . "&key=" . $this->getWXCONF('key')));
		$xml .= "<sign>{$sign}</sign>";
		$xml .= "</xml>";
		\Think\Log::write($xml);
		$str = $this->http_post('https://api.mch.weixin.qq.com/pay/unifiedorder', $xml);
		...
		if($xml['return_code'] == 'SUCCESS' && $xml['result_code'] == 'SUCCESS'){
			...
			if ($app == 4) {
				$data = [
					...
					'package' => 'prepay_id=' . $xml['prepay_id'], // 统一支付接口返回的prepay_id参数值，提交格式如：prepay_id=***）
			...
			*/
			...
			$data['sign'] = strtoupper(md5(join('&', $signArr) . "&key=" . $this->getWXCONF('key')));
			if(!$app)
				return $data;
			else{
				return [
					'appid' => $app_id,
					'partnerid' => $xml['mch_id'],
			...
			\Think\Log::write(json_encode($xml));
			$this->errCode = $xml['return_code'];
			$this->errMsg = dump($xml, false);
			return false;
		}
	}

	public function payToUser($openid, $amount, $desc){
		$url = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers';
		$data = [
			'mch_appid' => $this->appid,
			'mchid' => $this->getWXCONF('mchid'),
			'nonce_str' => $this->generateNonceStr(),
			'partner_trade_no' => createCode(40),
			'openid' => $openid,
			'check_name' => 'NO_CHECK',
			'amount' => $amount * 100,
			'desc' => $desc,
			'spbill_create_ip' => '120.25.165.87'
		];
	 ...
	 * 发送退款信息
	 */
	public function sendRefundInfo($orderSn, $total_fee, $refund_fee, $transaction_id = ''){
		...
		$data = [
			...
			'op_user_id' => $this->getWXCONF('mchid'),
			'appid' => $this->appid,
			'mch_id' => $this->getWXCONF('mchid'),
			'nonce_str' => $this->generateNonceStr(),
			'transaction_id' => ''
		...
		$rs = curl_post('https://api.mch.weixin.qq.com/secapi/pay/refund', $xml, [], true);
		return $rs;
	}
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		...
		//dump('Data Input : ' . $cachename . ' => ' . $value);
		return \Common\Util\Cache::getInstance()->set($cachename,$value,$expired);
	}
	...
	protected function getCache($cachename){
		//dump('Data Output : ' . $cachename);
		return \Common\Util\Cache::getInstance()->get($cachename);
	}
	...
	protected function removeCache($cachename){
		return \Common\Util\Cache::getInstance()->rm($cachename);
	}
		...
		if ($result)
		{
			...
			if (!$json || !empty($json['errcode'])) {
				if($json['errcode'] == 40001){
					$this->removeCache('wechat_access_token'.$appid);
					$this->checkAuth();
	 ...
	 * params $isapp 是否是微信小程序
	 * @return array {access_token,expires_in,refresh_token,openid,scope}
	 */
	public function getOauthAccessToken($code){
		$this->errMsg = '无法获取code';
