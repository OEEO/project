function myScroll(type){
	...
	this.start = function(){
		...
		for(var i in this.src){
			...
			if(o.type == 1){
				var $em = $('<em></em>').css({'width':'0.8rem','height':'0.8rem','border-radius':'50%','display':'inline-table','margin':'1.1rem 0.5rem','background':'rgba(255,255,255,0.6)'}).appendTo($dots);
		...
		if(o.type == 1){
			$dots.css({'position':'absolute','width':'100%','height':'3rem','z-index':2,'bottom':0,'left':0,'right':0,/*'background':'rgba(0,0,0,0.2)',*/'text-align':'center','overflow':'hidden'}).appendTo($main);
		}
