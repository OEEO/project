        ...
        $.ajax({
            ...
            success : function(code){
                ...
                if(window[pagename + 'Object'].onload && typeof(window[pagename + 'Object'].onload) == 'function'){
                    ...
                    (function LoadTimming(){
                        ...
                        if(typeof(fn) == 'function'){
                            ...
                            $('#header,#footer,#main,#fixed').find('.page_' + pagename).show();
                            win.close_loading();
                ...
                if(typeof(fn) == 'function'){
                    ...
                    $('#header,#main,#fixed').find('.page_' + pagename).show();
...
var page = {
    ...
    back: function (fn, callback) {
        if (page.num > 0) {
            ...
            if ($('.page_' + page.names[page.num]).size() > 0) {
                $('#header, #main, #footer, #fixed').find('.page_' + page.names[page.num]).show();
                $('.shareBox').hide();
                page.move(1, function () {
                    var name = page.names[page.num];
    ...
    move: function (way, fn) {
        ...
        $('#header, #main, #footer, #fixed').show().children().hide();
        setTimeout(function () {
            $('#header, #main, #footer, #fixed').find(".page_" + page.names[page.num]).show();
            setTimeout(function () {
                win.close_loading();
