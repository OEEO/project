    ...
    win.payInterval = setInterval(function(){
        try {
            ...
            if(days > 0 || hours > 0 || mins > 0 || secs > 0){
                ...
                $('#dialogBox.payBox .remaintime').html('支付时间还剩 <font>'+str+'</font>');
                win.paytimes --;
            } else {
                clearInterval(win.payInterval);
...
//微信支付
function wxpay(order_id, price, limit_time, type,gid,snum){
    type = type||0;
    var groups_id = gid || '';
    var snumber = snum || '';
    var code ='';
    ...
    code += '<p class="remaintime">支付时间还剩</p>';
    ...
    $.dialog(code, function(){
        if($('#dialogBox.payBox .paytype .alipay .selected').hasClass('on')){
            console.log('http://' + DOMAIN + '/order/pay/submitAlipay.do?token='+ win.token +'&order_id=' + order_id + '&type=' + type);
            window.location.href = 'http://' + DOMAIN + '/order/pay/submitAlipay.do?token='+ win.token +'&order_id=' + order_id + '&type=' + type;
        }else{
                                ...
                                if(res.errMsg == 'chooseWXPay:ok'){
                                    $.alert('支付成功', function(){
                                        if(type == 0){
                                            ...
                                            jump('choice-ucenter-myRaiseOrder-orderRaiseDetail', {order_id:order_id});
                                        }else{
                                            if(snumber != ''){
                                                //开团
