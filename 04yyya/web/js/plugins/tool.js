...
//		path = 'http://img.m.yami.ren/' + path;
		path = 'http://img.test.yummy194.cn/' + path;
	}
	return path;
};
 ...
 * @param is_cancel 可否取消
 */
$.dialog = function(msg, fn, is_lock, classname, is_cancel){
	if(!is_cancel){
		is_cancel = true;
	}
	...
	if (is_cancel) {
		...
		//如果在使用dialog时不传入cancel参数，则会在关闭时跳转至该分类相关的订单详情页面
		$('#dialogBox button.closeBtn, .dialogBoxLay, .clearpsd, .noticeid').click(function(){
			box.css({'opacity':0,'margin-top': -1 * (box.height()+h)});
			dialogBoxLay.css('opacity', 0);
			setTimeout(function(){
...
/**
 * 日期表
 * @param opt {days:[圈定的日期], time:<指定时间戳>, onselect:<点击日期函数回调,参数:年,月,日>}
 * @returns {$}
 */
$.fn.datepicker = function(opt){
	var signDays = opt.days||[];
	if(opt.time){
		var date = new Date(opt.time * 1000);
	}else{
		var date = new Date();
	}
	$(this).empty();
	var em = {};
	em.title = $('<div>').addClass('date-title').appendTo(this);
	em.table = $('<table>').appendTo(this);
	...
	function create(){
		...
		var w = date.getDay();
		date.setDate(40);
		var end = 40 - date.getDate();
		date.setDate(1);
		var days = [];
		for(var i in signDays){
			var arr = signDays[i].split('-');
			if(date.getFullYear() == parseInt(arr[0]) && date.getMonth() == parseInt(arr[1])){
				days.push(parseInt(arr[2]));
			}
		}
		//表格
		var code = '<tr><th>日</th><th>一</th><th>二</th><th>三</th><th>四</th><th>五</th><th>六</th></tr>';
		var d = 0;
		for(var i=1; i<=5; i++){
			code += '<tr>';
			for(var j=1; j<=7; j++){
				...
				if(d >= 1 && d <= end){
					...
					code += '<td class="'+ classes.join(' ') +'"><span>'+ d +'</span></td>';
					d ++;
				}else{
					code += '<td></td>';
				}
		...
		em.table.find('td.selected').on('click', function(){
			date.setDate($(this).text());
			em.table.find('.now').removeClass('now');
			$(this).addClass('now');
			if(typeof(opt.onselect) == 'function')opt.onselect(em.year.val(), em.month.val(), $(this).text());
		});
	}

	create();
	em.year = $('<select>').addClass('date-year').appendTo(em.title);
	for(var i=date.getFullYear() - 5; i<=date.getFullYear()+5; i++){
		em.year.append('<option value="'+ i +'">'+ i +'年</option>');
	}
	em.year.val(date.getFullYear());
	em.month = $('<select>').addClass('date-month').appendTo(em.title);
	for(var i=1; i<=12; i++){
		em.month.append('<option value="'+ i +'">'+ i +'月</option>');
	}
	em.month.val(date.getMonth());

	em.year.on('change', function(){
		date.setFullYear(this.value);
		create();
	});
	em.month.on('change', function(){
