				...
				for(var i in d){
					...
					if(d[i].act_status == 0){
						if(parseInt(d[i].price) > 2000 || win.get.android){
                            ...
                            var param = [];
							if (d[i].piece_info) {
                                param = [d[i].id, d[i].price, d[i].limit_time, d[i].type, d[i].piece_info.piece_originator_id, -1, '\{pieceType: 2, group_id: ' + d[i].piece_info.piece_originator_id + '\}'];
                            } else {
								param = [d[i].id, d[i].price, d[i].limit_time, d[i].type];
							}
							code += '		<a href="javascript:wxpay('+ param.join(',') + ')" class="btn">立即付款</a>';
						}
                        ...
                        if (d[i].piece_info) {
                            d[i].piece_info.act_status === '3'
                                ? code += '<span class="groups_status">(拼团成功)</span>'
                                : code += '<span class="groups_status">(等待拼团)</span>';
                        }

						if (d[i].is_piece == 1) {
							code += '<a href="javascript:jump(\'groupsDetail\',{type: 1, groups_id:' + d[i].piece_info.piece_originator_id + '});" class="btn primary">组团详情</a>';
						}
                        ...
                        code += '		<a href="javascript:myGoodsOrderObject.logistics('+ d[i].id +');" class="btn">查看物流</a>';
					}else if(d[i].act_status == 5 && !d[i].piece_info){
