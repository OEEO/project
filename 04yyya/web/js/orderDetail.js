		...
		ajax('Member/Order/getDetail', {order_id : orderDetailObject.order_id}, function(d){
			...
			if(d.act_status == 0){
				...
				if(d.piece_originator_id != ''){
					$em.find('.orderOperation').html('拼团详情').click(function(){
						jump('groupsDetail',{groups_id:d.piece_originator_id});
					});
				}else{
				...
				if (d.act_status == 1) {
					...
					if (Date.now() / 1000 < d.end_time) {
                        ...
                        $('.page_orderDetail.orderBottom').addClass('disabled');
					}
