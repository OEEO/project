var groupsDetailObject = {
            ...
            if (d.title) {
                ...
                if(d.other && d.other.length > 0){
                    ...
                    for(var i in d.other){
                        var item = d.other[i];
                        code += '<a href="javascript:;" class="item"><img src="' + item.path + '" alt="" onerror="goodsDetailObject.nofound();">';
                    ...
                    $('.page_groupsDetail .recommend .tips_list').html(code);
                }else{
                    $('.page_groupsDetail .recommend').remove();
                }
            }
        });
    },
    ...
    onload:function(){
        ...
        ajax('Member/Piece/Index', {'piece_originator_id':groupsDetailObject.groups_id, 'type': groupsDetailObject.type}, function(d){
            ...
            if (d.member_id === d.self_member_id && +d.piece_count > d.joiner.length) {
                $('#fixed,.page_groupsDetail.groups_success').show();
                $('.page_groupsDetail.groups_success .num').html(+d.piece_count - d.joiner.length);
            }
            ...
            var title = '【还差' + num + '人】我' + d.piece_price + '元拼了' + d.title + '快来和我一起拼团吧!';
            // var desc = d.edge[0];

            var desc = d.title + '吖咪限时拼团中!';
            // var title = d.title;
           ...
           // share(member.nickname + '邀请你一起来拼团，'+ d.piece_count +'人参与享特惠！', desc, url, d.type_path);
            ...
            // share(d.title +'，只要'+ d.piece_price + '元，点击和我一起拼！', desc, url, d.type_path);
            share(title, desc, url, d.type_path);
            ...
            $('.page_groupsDetail .orderListRight .t_c').text('原价：' + (+d.price) + '元/份');
            ...
            if(parseInt(d.end_time) > groupsDetailObject.time || parseInt(d.piece_count) <= parseInt(d.buyer_num) || ['1','2'].indexOf(d.act_status) !== -1){
                if(parseInt(d.end_time) > groupsDetailObject.time && d.piece_count > d.buyer_num){
                ...
                if(d.is_buy == 1){
                    ...
                    if (d.joiner.length >= d.piece_count && ['1','2','3'].indexOf(d.act_status) !== -1) {
                        // 拼团成功
            ...
            if(d.joiner.length > 0){
                ...
                if (+d.piece_count > d.joiner.length) {
                    for (var j = 0, num2 = +d.piece_count - d.joiner.length; j < num2; j++) {
                        code2 += '<div class="group-imgs-item" style="background: transparent url(http://yummy194.cn/images/xxx@2x.jpg) no-repeat 100% / 100%"></div>';
                    }
                }
