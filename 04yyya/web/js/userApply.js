...
var userApplyObject = {
    ...
    loadtitle:function(){
        ajax('Home/Research/GetDetail', {get:{token:win.token},post:{research_id:35}}, function(d){
            var code = '';
            for(var i in d){
                if(d[i].type == 0){
                    code += '<input type="text" name="'+d[i].value+'"  placeholder="'+d[i].content+'(必填)"/>';
                }else{
                    code += '<select name="'+d[i].value+'" onchange="userApplyObject.bian(this)">';
                    code += '<option value="" style="display:none;" disabled selected>'+d[i].content+'(必填)</option>';
