...
var raisePayObject = {
        ...
        ajax('Goods/Raise/getRaiseOrder', sdata, function(d){
            if(d.status == 1) {
                ...
                */
                if (d.info.data.is_address == 0) {
                    $('.page_raisePay .shop_address').remove();
                    $('.page_raisePay .pay_num').next('.the_blank').remove();
                }
    ...
    findUnpaid:function() {
        //        console.log(raisePayObject.data.raise_id);
 //       console.log(raisePayObject.data.times_id);
...
//            console.log(d.member_id);
         //   console.log(d.order_id);
            for (i in d.order_id) {
             ...
             //       console.log(raisePayObject.data.times_id);
                    if (d.order_id[i].ware_id == raisePayObject.data.raise_id && d.order_id[i].tips_times_id == raisePayObject.data.times_id) {
          //              console.log('good');
...
//        jump('choice-ucenter-myRaiseOrder');
    ...
    onload : function(){
        ...
        $('.page_raisePay .submitBtn').click(function(){
            ...
            //存储数量
            datas.num = raisePayObject.buy_num;
            ...
            $.dialog(code, function(){
                ...
                ajax('Order/Index/create', datas, function (d) {
                    if (d.status == 1) {
                        ...
                        // }else{
                        var price = raisePayObject.price * raisePayObject.buy_num;
        ...
        ajax('Goods/Raise/getLimit', {raise_id:raisePayObject.data.raise_id,times_id:raisePayObject.data.times_id}, function (d) {
            raisePayObject.limit = d.limit;
            raisePayObject.limit_buy_times = d.limit_buy_times;
//            console.log(raisePayObject.limit);
  //          console.log(raisePayObject.limit_buy_times);
//            console.log(d);
        });
        ajax('Goods/Raise/getStock', {raise_id:raisePayObject.data.raise_id,times_id:raisePayObject.data.times_id}, function (d) {
            raisePayObject.stock = d.stock;
//          console.log(raisePayObject.stock);
        });
/*
        if (window.location.href = 'http://yummy194.cn/?page=choice-raiseDetail&raise_id=78') {
            $('#dialogBox.payBox .paytype a.wx').removeClass('disable');
            $('#dialogBox.payBox .paytype a.alipay .selected').removeClass('on');
        }
        */
        //判断是否登录，没有登录则跳转登录
        if(!member){
    ...
    changeCopies : function (em, num){
        if (num > 0 && raisePayObject.limit_buy_times > 0 && raisePayObject.buy_num >= raisePayObject.limit_buy_num) {
            $.alert('每人限购' + raisePayObject.limit_buy_num + '份');
            return;
        }
        if (em) {
            var currentVal = raisePayObject.buy_num;
            if(currentVal <= 1 && num == '-1')num=0;
            if(currentVal >= raisePayObject.stock && num == '1' && raisePayObject.stock != '-1')num=0;
            if(currentVal >= raisePayObject.limit_buy_times && num == '1' && raisePayObject.limit != '0')num=0;
            currentVal += (+num);
            raisePayObject.buy_num = currentVal;
        } else {
            raisePayObject.buy_num = num;
        }

        // $(em).parent().children('.b').value(currentVal);
        $('.page_raisePay #num').val(raisePayObject.buy_num);
        raisePayObject.changePrice();
    },

    //改变价钱
    changePrice : function(){
        //raisePayObject.buy_price = parseFloat(raisePayObject.buy_price);
        var price = raisePayObject.price;
        var num = raisePayObject.buy_num;
        price *= num;
        price = price.toFixed(2);
       // raisePayObject.price = pric

        /*
        var _price = price;
        //计算优惠券
        if(raisePayObject.type == 0) {
            _price = Math.round((price - raisePayObject.value + raisePayObject.shipping) * 100) / 100;
            $('.page_raisePay #coupon_release_price').text("- ￥" + raisePayObject.value);
        } else if(raisePayObject.type == 1) {
            _price = price * raisePayObject.value / 100 + raisePayObject.shipping;
            $('.page_raisePay #coupon_release_price').text("- ￥" + price * (100 - raisePayObject.value) / 100);
        }
        ...
        raisePayObject.buy_price = _price;
            */
        $('.page_raisePay #buy_price').html(price);
       // $('.page_raisePay #number_price').html('￥' + raisePayObject.item_price + '<i>x</i>' + num);
    },
};
