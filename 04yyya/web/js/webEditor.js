var webEditorObject = {
	...
	setColor : function(color){
		...
		if(localStorage){
			var colors = [];
			if(storage.get('latelyColors'))colors = storage.get('latelyColors').split('|');
			colors.push(color);
	...
	onload : function(){
		...
		if(window.localStorage && storage.get('latelyColors')){
			var colors = storage.get('latelyColors').split('|');
			for(var i in colors){
				$('<button>').css('background-color',colors[i]).appendTo('.page_webEditor.tools .setColor .lately').on('click', function(){
					var color = $(this).css('background-color');
					webEditorObject.css(webEditorObject.name, color);
