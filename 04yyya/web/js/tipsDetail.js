var tipsDetailObject = {
			...
			try {
				...
				if(days > 0 || hours > 0 || mins > 0 || secs > 0){
					...
					$('.page_tipsDetail .timebox').hide();
						$('.page_tipsDetail.footer .coursebox .groups_price').css({'background':'#b39851'}).on('click', function(){
	...
	shareSuccess: function (id) {
		return function(target) {
            ajax('Home/Index/shareSuccess', {type: 0, item_id: id, target: target, platform: 0});
		}
	},
	onload : function(){
		...
		$('.page_tipsDetail .chat').click(function(){
			if(win.ws.power == 0){
		...
		$('.page_tipsDetail.wrapper').scroll(function(){
			if($(this).scrollTop() > $('.page_tipsDetail .bodyTop').height()){
				$('.page_tipsDetail.header').removeClass('changeStyle');
				$('.page_tipsDetail.header').addClass('line');
				$('.page_tipsDetail.header a.collect.c2').show();
				$('.page_tipsDetail.header a.collect.c1').hide();
				$('.page_tipsDetail.header a.shared').css('background','url("../images/course-detail-page_share1@2x.png") no-repeat 0 0 / 1.728rem 1.728rem');
			}else{
				$('.page_tipsDetail.header').addClass('changeStyle');
				$('.page_tipsDetail.header').removeClass('line');
				$('.page_tipsDetail.header a.collect.c2').hide();
				$('.page_tipsDetail.header a.collect.c1').show();
				$('.page_tipsDetail.header a.shared').css('background','url("../images/course-detail-page_share@2x.png") no-repeat 0 0 / 1.728rem 1.728rem');
			}
			//判断并悬浮菜单按钮
			if($(this).scrollTop() > $('.page_tipsDetail .menues .menu_lists').position().top - 45){
				$('.fixed.page_tipsDetail .menu_lists').show();
				// tipsDetailObject.menuClone = $('.page_tipsDetail .menu_lists').clone(true, true).appendTo('.fixed');
				// tipsDetailObject.menuClone.addClass('menu_lay');
			}
			//判断并清除菜单按钮
			if($(this).scrollTop() < $('.page_tipsDetail .menues .menu_lists').position().top - 45){
		...
		ajax('Goods/Tips/getDetail', {'tips_id':tipsDetailObject.tips_id}, function(d){
			...
			if((d.address && d.price!=null && d.time && d.times && d.title && d.min_num!=null && d.restrict_num!=null) || location.href.indexOf('tipsDetail') > 0){
				...
				share(d.title, desc, url, d.mainpic, tipsDetailObject.shareSuccess(tipsDetailObject.tips_id));

				tipsDetailObject.defaultPics = d.defaultPics;
				//分享绑定
				$('.page_tipsDetail .shared').click(function(){
				...
				$('.page_tipsDetail .edges').html(edges);
					...
					// $('.page_tipsDetail.footer .dinnerbox').hide();
					// $('.page_tipsDetail.footer .coursebox').show();
					// $('.page_tipsDetail .them_toptitle .share').show();
					// $('.page_tipsDetail .them_toptitle .collect').show();
					//是否拼团
					 if(d.time.piece.length > 0){
					 	$('.page_tipsDetail.footer .coursebox .original_price').html('<big>￥<font>'+d.price+'</font></big><small>原价购买</small>');
					 	$('.page_tipsDetail.footer .coursebox .groups_price').html('<big>￥<font>'+d.time.piece[0].price+'</font></big><small>拼团购买</small>');
					 }else{
						$('.page_tipsDetail.footer .coursebox .original_price font').html(d.price);
						$('.page_tipsDetail.footer .coursebox .groups_price').html('JOIN');
					 }
				//成局
				...
				//2018.2.25
				 if(d.time.piece.length > 0){
				 	$('.page_tipsDetail .b_start').attr('groups_id',d.time.piece[0].id);
				 }
						 ...
						 if(d.times[i].piece.length > 0){
						 	for(var m in d.times[i].piece){
						 		if(d.times[i].piece[m].can_buy == 1){
						 			if(g== 0){
						 				code +='<div class="t_right yes" start_time="'+ d.times[i].start_time +'" end_time="'+ d.times[i].end_time +'" i="'+ i +'" onclick="tipsDetailObject.selectTime(this)" stop_buy_time="'+ d.times[i].stop_buy_time +'" start_buy_time="'+ d.times[i].start_buy_time +'" time_id="'+ d.times[i].id +'" groups_id="'+ d.times[i].piece[0].id +'" groups_price="'+ d.times[i].piece[m].price +'"></div>';
						 				g++;
						 			}
						 		}
 						 	}
				...
				$('.page_tipsDetail .time_but').click(function(){
					$('.page_tipsDetail.footer .coursebox .groups_price').off('click');
					...
					//2018.2.25
					//判断是否有开团id
					 if(typeof($(".page_tipsDetail .select").attr("groups_id")) != "undefined"){
					 	var groups_id = $('.page_tipsDetail .select').attr('groups_id');
					 	var groups_price = $('.page_tipsDetail .select').attr('groups_price');
					// 	// $('.page_tipsDetail .b_start').attr('groups_id',groups_id);
					 }
					// }
					...
					if(parseInt(start_buy_time) > (new Date()).getTime() / 1000){
						...
						//2018.2.25
						 if(groups_id){
						 	$('.page_tipsDetail.footer .coursebox .original_price').css({'background':'#ccc'}).html('<big>￥<font>'+d.price+'</font></big><small>原价购买</small>');
						 	$('.page_tipsDetail.footer .coursebox .groups_price').css({'background':'#bbb'}).html('<big>￥<font>'+groups_price+'</font></big><small>拼团购买</small>');
						 	$('.page_tipsDetail.footer .coursebox .original_price').off('click');
						 	$('.page_tipsDetail.footer .coursebox .groups_price').off('click');
						 }else{
							$('.page_tipsDetail.footer .coursebox .original_price').css({'background':'#ccc'}).html('<font>'+d.price+'</font>元');
							$('.page_tipsDetail.footer .coursebox .groups_price').css({'background':'#bbb'}).html('JOIN');
							// $('.page_tipsDetail.footer .coursebox .original_price').off('click');
							$('.page_tipsDetail.footer .coursebox .groups_price').off('click');
						 }
						...
						$('.page_tipsDetail .timebox').hide();
						 if(groups_id){
						 	$('.page_tipsDetail.footer .coursebox .original_price').css({'background':'#bea76b'}).html('<big>￥<font>'+d.price+'</font></big><small>原价购买</small>').on('click', function(){
								tipsDetailObject.submitOrder();
						 	});
						 	$('.page_tipsDetail.footer .coursebox .groups_price').css({'background':'#b39851'}).html('<big>￥<font>'+groups_price+'</font></big><small>拼团购买</small>').on('click', function(){
						 		tipsDetailObject.groupsOrder();
						 	});
						 }else{
						// 	$('.page_tipsDetail.footer .coursebox .original_price').off('click');
							$('.page_tipsDetail.footer .coursebox .original_price').css({'background':'#bea76b'}).html('<font>'+d.price+'</font>元');
							$('.page_tipsDetail.footer .coursebox .groups_price').css({'background':'#b39851'}).html('JOIN').on('click', function(){
								tipsDetailObject.submitOrder();
							});
						 }
					}
				...
				if($('.page_tipsDetail .center_list .t_left').not('.none').size() > 0){
					...
					if(d.time.stock <= 0){
						tipsDetailObject.can_buy_time = 0;
						 if(d.time.piece.length > 0){
						 	$('.page_tipsDetail.footer .coursebox .original_price').off('click').css('background','#ccc');
						 	$('.page_tipsDetail.footer .coursebox .groups_price').off('click').css('background','#bbb');
						 }else{
							$('.page_tipsDetail.footer .coursebox .original_price').css('background','#ccc');
							$('.page_tipsDetail.footer .coursebox .groups_price').off('click').css('background','#bbb');
						 }
