var orderGoodsDetailObject = {
	...
	onload : function(){
		...
		ajax('Member/Order/getDetail', {order_id : orderGoodsDetailObject.order_id}, function(d){
			...
			var desc = d.title;
			var url = win.host + '?page=choice-goodsDetail&goods_id=' + d.goods_id;
			...
			if(d.act_status == 0){
				...
				$('.page_orderGoodsDetail.orderBottom').text('立即付款').click(function(){
					wxpay(orderGoodsDetailObject.order_id, d.total, d.limit_time, 1, d.piece_originator_id, -1, {pieceType: 2, group_id: d.piece_originator_id});
				});

			}else if(d.act_status == 1){
                if (win.get.pieceType == 1 && d.piece_originator_id) {
                    $.alert('恭喜你开团成功');
                }

				$em.find('.orderOperation').empty();
				
				if (d.piece_originator_id) {
					url = win.host + '?page=groupsDetail&groups_id=' + d.piece_originator_id;
				}
			...
			share(d.title, desc, url, d.path); // 分享
