...
var hiddenRaisePayObject = {
    ...
    is_certification:function(){
        ajax('Goods/Raise/getRaiseReal', {}, function(d){
                if (d.status == 0) {
                    $('.page_hiddenRaisePay .certification .cbox').on('click', function () {
                        hiddenRaisePayObject.certification();
                    });
                } else {
                    $('.page_hiddenRaisePay .certification .cbox').off('click');
                    $('.page_hiddenRaisePay .certification .cbox').css('color','#333');
                    $('.page_hiddenRaisePay .certification .cbox').html(d.surname + ' ' + d.identity);
                    hiddenRaisePayObject.cefi = d.surname + ' ' + d.identity;
                }
        });
    },
    //判断微信号
    is_wx:function(){
        ajax('Member/Index/info', {}, function(d){
            if(window.sessionStorage.getItem('address_id') != '' && window.sessionStorage.getItem('address_id') != null){
                $('.page_hiddenRaisePay .shop_address .add').html(window.sessionStorage.getItem('address'));
                hiddenRaisePayObject.address_id = window.sessionStorage.getItem('address_id');
            }
            if(d.default_address_id){
                hiddenRaisePayObject.address_id = d.default_address_id;
                $('.page_hiddenRaisePay .shop_address .add').html(d.default_address);
                $('.page_hiddenRaisePay .shop_address .add').css('color','#333');
            }else{
                    hiddenRaisePayObject.address_id = ' ';

            }
            if (d.weixincode != null && d.weixincode !='') {
                hiddenRaisePayObject.wx_id = d.weixincode;
                $('.page_hiddenRaisePay .wxinput .wxcode').css('color','#333');
                $('.page_hiddenRaisePay .wxinput .wxcode').val(d.weixincode);
                $('.page_hiddenRaisePay .wxinput .wxcode').attr('readonly','readonly');
    ...
    addressInput : function(id, name, tel, address){
        hiddenRaisePayObject.address_id = id;
        $('.page_hiddenRaisePay .shop_address .add').html(address);
        $('.page_hiddenRaisePay .shop_address .add').css('color','#333');
    },
    //实名认证
        ...
        $.dialog(c, function(){
            ...
            ajax('Goods/Raise/raise_real', data, function(d){
                if(d.status == 1){
                    $.alert(d.info);
                    $('.page_hiddenRaisePay .certification .cbox').css('color','#333');
                    $('.page_hiddenRaisePay .certification .cbox').html(nick + ' ' + id);
                    hiddenRaisePayObject.cefi = nick + ' ' + id;
    ...
    raisestep:function(){
        var sdata = {};
        sdata.raise_id = hiddenRaisePayObject.data.raise_id;
        sdata.raise_times_id = hiddenRaisePayObject.data.times_id;
        ajax('Goods/Raise/getRaiseOrder', sdata, function(d){
            if(d.status == 1) {
                hiddenRaisePayObject.price = d.info.data.pay_price;
                $('.page_hiddenRaisePay .raiseitem .title').html(d.info.data.title);
                $('.page_hiddenRaisePay .raiseitem .money').html('￥' + d.info.data.pay_price);
                $('.page_hiddenRaisePay.payMenu .paymoney [name="money"]').html(d.info.data.pay_price);
                //当支付金额超过2000,则引导至支付宝支付
                if (parseFloat(d.info.data.pay_price) > 2000 || win.get.android) {
                    $('.page_hiddenRaisePay .paytype a.wx').addClass('disable');
                    $('.page_hiddenRaisePay .paytype a.alipay ').click();
                }
                if (d.status == 0) {
                    $.alert(d.info,'error');
                    return;
                ...
                if (d.info.data.is_address == 0) {
                    $('.page_hiddenRaisePay .shop_address').remove();
                    $('.page_hiddenRaisePay .raiseitem').next('.the_blank').remove();
                }
                */
                if (d.info.data.is_address == 0) {
                    $('.page_hiddenRaisePay .shop_address').remove();
                    $('.page_hiddenRaisePay .pay_num').next('.the_blank').remove();
                }
                if (d.info.data.is_realname == 0) {
                    $('.page_hiddenRaisePay .certification').remove();
                    $('.page_hiddenRaisePay .wx_num').next('.the_blank').remove();
                }
                ...
                $('.page_hiddenRaisePay .stage .stageitem').html(scode);
            }else{
                $.alert(d.info,'error');
            }
        });
    ...
    findUnpaid:function() {
        //        console.log(hiddenRaisePayObject.data.raise_id);
 //       console.log(hiddenRaisePayObject.data.times_id);
...
//            console.log(d.member_id);
         //   console.log(d.order_id);
            ...
            for (i in d.order_id) {
             ...
             //       console.log(hiddenRaisePayObject.data.times_id);
                    if (d.order_id[i].ware_id == hiddenRaisePayObject.data.raise_id && d.order_id[i].tips_times_id == hiddenRaisePayObject.data.times_id) {
                    //    console.log('good');
...
//        jump('choice-ucenter-myRaiseOrder');
    ...
    onload : function(){
        hiddenRaisePayObject.data = win.get;
        hiddenRaisePayObject.is_certification();
        hiddenRaisePayObject.raisestep();
        hiddenRaisePayObject.is_wx();

        hiddenRaisePayObject.findUnpaid();

        //console.log(hiddenRaisePayObject.data);
        //更多地址按钮
        $('.page_hiddenRaisePay .shop_address .address').on('click',function(){
            jump('myAddress', {address_id: hiddenRaisePayObject.address_id});
        });

        //支付方式
        $('.page_hiddenRaisePay .paytype a').on('click', function(){
            if(!$(this).hasClass('disable')){
                $('.page_hiddenRaisePay .paytype a .selected').removeClass('on');
                $(this).find('.selected').addClass('on');
            }
        });
        $('.page_hiddenRaisePay .submitBtn').click(function(){
            var datas = {};

            //存储数量
            datas.num = hiddenRaisePayObject.buy_num;

            datas.raise_id = hiddenRaisePayObject.data.raise_id;
            datas.times_id = hiddenRaisePayObject.data.times_id;
            if($('.page_hiddenRaisePay .shop_address').length > 0){
                datas.address_id = hiddenRaisePayObject.address_id;
            }
            if(hiddenRaisePayObject.data.id){
                datas.member_privilege_id = hiddenRaisePayObject.data.id;
            }
            datas.weixincode = $('.page_hiddenRaisePay .wxinput .wxcode').val();
            datas.oper_read = '1';
            for(var i in datas){
                if(datas[i] == '' || datas[i] == ' '){
                    $.alert('请完善信息','error');
            ...
            if($('.page_hiddenRaisePay .certification').length > 0){
                if(hiddenRaisePayObject.cefi == ''){
                    $.alert('请完善信息','error');
                    return;
                }
            }
            ...
            $.dialog(code, function(){
                ...
                ajax('Order/Index/create', datas, function (d) {
                    if (d.status == 1) {
                        hiddenRaisePayObject.order_id = d.info.order_id;
                        // if($('.page_hiddenRaisePay .paytype .alipay .selected').hasClass('on')){
                        //     window.location.href = 'http://' + DOMAIN + '/order/pay/submitAlipay.do?token='+ win.token +'&order_id=' + d.info.order_id;
                        // }else{
                        var price = hiddenRaisePayObject.price * hiddenRaisePayObject.buy_num;
                        price = price.toFixed(2);
                        //wxpay(d.info.order_id,hiddenRaisePayObject.price,d.info.limit_pay_time,2);
                        wxpay(d.info.order_id,price,d.info.limit_pay_time,2);
                        // }
                    } else {
                                ...
                                ajax('Home/Wx/getOauthUrl', function (d) {
                                    if (typeof(d) == 'string') {
                                        if (window.sessionStorage) {
                                            window.sessionStorage.setItem('address', $('.page_hiddenRaisePay .shop_address .add').text());
                                            window.sessionStorage.setItem('address_id', hiddenRaisePayObject.address_id);
                                            window.sessionStorage.setItem('jumpUrl', 'page=hiddenRaisePay&raise_id=' + hiddenRaisePayObject.data.raise_id + '&times_id=' + hiddenRaisePayObject.data.times_id);
                                        }
    ...
    changeCopies : function (em, num){
        if (num > 0 && hiddenRaisePayObject.limit_buy_num > 0 && hiddenRaisePayObject.buy_num >= hiddenRaisePayObject.limit_buy_num) {
            $.alert('每人限购' + hiddenRaisePayObject.limit_buy_num + '份');
            return;
        }

        if (em) {
            var currentVal = hiddenRaisePayObject.buy_num;
            if(currentVal <= 1 && num == '-1')num=0;
            if(currentVal >= hiddenRaisePayObject.stocks && num == '1')num=0;
            currentVal += (+num);
            hiddenRaisePayObject.buy_num = currentVal;
        } else {
            hiddenRaisePayObject.buy_num = num;
        }

        // $(em).parent().children('.b').value(currentVal);
        $('.page_hiddenRaisePay #num').val(hiddenRaisePayObject.buy_num);
        hiddenRaisePayObject.changePrice();
    },

    //改变价钱
    changePrice : function(){
        //hiddenRaisePayObject.buy_price = parseFloat(hiddenRaisePayObject.buy_price);
        var price = hiddenRaisePayObject.price;
        var num = hiddenRaisePayObject.buy_num;
        price *= num;
        price = price.toFixed(2);
       // hiddenRaisePayObject.price = pric

        /*
        var _price = price;
        //计算优惠券
        if(hiddenRaisePayObject.type == 0) {
            _price = Math.round((price - hiddenRaisePayObject.value + hiddenRaisePayObject.shipping) * 100) / 100;
            $('.page_hiddenRaisePay #coupon_release_price').text("- ￥" + hiddenRaisePayObject.value);
        } else if(hiddenRaisePayObject.type == 1) {
            _price = price * hiddenRaisePayObject.value / 100 + hiddenRaisePayObject.shipping;
            $('.page_hiddenRaisePay #coupon_release_price').text("- ￥" + price * (100 - hiddenRaisePayObject.value) / 100);
        }
        ...
        hiddenRaisePayObject.buy_price = _price;
            */
        $('.page_hiddenRaisePay #buy_price').html(price);
       // $('.page_hiddenRaisePay #number_price').html('￥' + hiddenRaisePayObject.item_price + '<i>x</i>' + num);
    },
};
