...
//			return '即将上线';
	...
	buildRaiseItem: function (d) {
        ...
        code += '<span style="width:12rem;">已认筹￥<font name="sum">' + (d.totaled || 0) + '</font></span>';

        var end = +d.end_time;
        var time = choiceObject.getLastTime(cur, end, +d.start_time, null, d.totaled - d.total);
        if (cur < +d.start_time && !isPreview) {
            time = '预约中';
        }
	...
	loadtips: function (data) {
		var code = '';
		/*
        ajax('Goods/Raise/getNew', { get : {page :page} }, function(d) {
            if (d.length > 0) {

                var cur = parseInt(Date.now() / 1000);

                code += '<div class="items" onclick="javascript:$.alert(\'即将上线，敬请期待！\')">';
                code += '<img class="raiseimg" src="' + d[0].path + '"/>';
                code += '<div class="items-top">';
                code += '<div class="raise-city"><img src="../images/row_button.png" />' + d[0].city_name + '</div>';
                code += '<span class="name">' + d[0].nickname + '</span>';
                // code +=         '<span class="address">广州</span>';
                code += '</div>';
                code += '<div class="raise-title">' + d[0].title + '</div>';
                code += '<p class="dec">' + d[0].introduction + '</p>';

                var per = +d[0].total === 0 ? 0 : (+d[0].totaled / +d[0].total)  * 100;
                per = per.toFixed(2);
                code += '<div class="shell">';
                code += '<div class="c_line">';
                code += '<span style="width: ' + per + '%;"></span></div></div>';
                ...
                code += '<span style="width:12rem;">已认筹￥<font name="sum">' + (d[0].totaled || 0) + '</font></span>';

                var end = +d[0].end_time;

                code += '<span name="days">即将上线</span></div>';
        ...
        */
		...
		setTimeout(function(){
			...
			ajax('Goods/Home/getHomeList', {get:{page:choiceObject.tips_page}, post:choiceObject.tips_data}, function(d){
				$('.page_choice center').hide();
//				var code = '';
				...
				// }
	            code += '<li class="the_end"><div class="no_more"></div><div style="margin:1rem 0 0 0;"><a href="http://www.miitbeian.gov.cn">备案号:粤ICP备15026446号-1</div></li>';
				if(choiceObject.tips_page == 1)
