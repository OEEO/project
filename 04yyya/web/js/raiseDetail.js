var raiseDetailObject = {
    ...
    onload : function(){
        ...
        $('.page_raiseDetail .status_check a').click(function(){
            ...
            // });
//            $('.page_raiseDetail.wrapper').animate({'scrollTop':div.position().top - 25}, 'fast', function(){
            var str = $(this).attr('data');
            $('.page_raiseDetail .status_check a').removeClass('add_hei');
            $('.page_raiseDetail .status_check a[data="'+ str +'"]').addClass('add_hei');
            $('.page_raiseDetail .dec>.item').hide();
            $('.page_raiseDetail .dec>.' + str).show();
        });

        $('#header .header.line.page_raiseDetail .turnBack .arrow').css('border-left', '0.15rem solid white')
        ...
        $('.page_raiseDetail.wrapper').scroll(function(){
            ...
            if($(this).scrollTop() > $('.page_raiseDetail .c_video').height()) {
                $('#header .header.line.page_raiseDetail').css('background', 'white').css('border-bottom', '0.1rem solid #eee');
                ...
                $('#header .header.line.page_raiseDetail .turnBack .arrow').css('border-left', '0.15rem solid #000')
                .css('border-top', '0.15rem solid #000');
                $('.page_raiseDetail .userMsg').css('background', "url('../images/homepage_user_icon@3x.png') no-repeat center center / 2.2rem 2.2rem");
            }
            if($(this).scrollTop() < $('.page_raiseDetail .c_video').height()) {
                $('#header .header.line.page_raiseDetail').css('background', 'none').css('border-bottom', '0rem');
                ...
                $('#header .header.line.page_raiseDetail .turnBack .arrow').css('border-left', '0.15rem solid white')
                .css('border-top', '0.15rem solid white');
                $('.page_raiseDetail .userMsg').css('background', "url('../images/homepage_user_icon_w@3x.png') no-repeat center center / 2.2rem 2.2rem");
            }
        });
...
//                     $('.fixed_top.page_raiseDetail .statusbox').fadeIn();
//                     if ($('.statusbox').css('display') == 'block') {
//                         clearTimeout(raiseDetailObject.timeoutId);
//                         raiseDetailObject.timeoutId = setTimeout("$('.statusbox').fadeOut()",2000);
//                     }
//                     /*
//                     if($('.statusbox').css('display') == 'block') {
...
//             }
//             if($(this).scrollTop() < ($('.detail').position().top - 80)){
//                 $('.fixed_top.page_raiseDetail .statusbox').fadeOut();
//             }
//             */
//             /*
//             //判断并清除菜单按钮
//             if($(this).scrollTop() < $('.page_raiseDetail .status_check').position().top - 40){
//                 $('.fixed_top.page_raiseDetail .statusbox').hide();
...
//         });
        ...
        ajax('Goods/Raise/getDetail', {'raise_id':raiseDetailObject.raise_id}, function(d){
            ...
            var isEnd = parseInt(d.end_time) < raiseDetailObject.time ? true : false;
            ...
            //尚未开始倒计时
//            if(d.isPrivilege=="0"&&parseInt(d.start_time) > raiseDetailObject.time){
            if(parseInt(d.start_time) > raiseDetailObject.time && !isPreview){
                //未开始，且非预告，显示倒计时
            ...
            if(parseInt(d.end_time) < raiseDetailObject.time){
                days = '已结束';
            }else if(d.isPrivilege==0&&parseInt(d.start_time) > raiseDetailObject.time){
                console.log(d.isPrivilege);
//              days = '未开始';
                days = '优先认筹中';
            }else{
                var t = parseInt(d.end_time) - raiseDetailObject.time;
                days = '<font>剩余' + raiseDetailObject.getLastTime(raiseDetailObject.time, d.end_time, d.start_time) + '</font>';
            }*/
            console.log('isPreview = ', isPreview);
            console.log('isCollect = ', isCollect);
            if(parseInt(d.end_time) < raiseDetailObject.time){
                days = '已结束';
                $('.page_raiseDetail .suport-btn').attr({'disabled':true});
            }else if(parseInt(d.start_time) > raiseDetailObject.time){
                $('.page_raiseDetail .collected-count').show().text('关注人数：' + d.collected_count);
                if (isPreview) {
                    $('.page_raiseDetail .suport-btn').remove();
                    $('.page_raiseDetail .follow-btn').show();
                    days = '上线时间待公布';
                }

                if (isReminder) {
                    $('.page_raiseDetail .follow-btn').addClass('is-collect').text('已关注');
                }
                console.log(d.isPrivilege);
//              days = '未开始';
                days = '预约中';
                ...
                var t = parseInt(d.end_time) - raiseDetailObject.time;
                days = '<font>' + raiseDetailObject.getLastTime(raiseDetailObject.time, d.end_time, d.start_time) + '</font>';
            }
            ...
            $('.page_raiseDetail .nickname').text(d.nickname);
            $('.page_raiseDetail .city_name').text(d.city_name);
            $('.page_raiseDetail .c_time').text(d.start_time.timeFormat("Y年m月d日"));
            $('.page_raiseDetail .raise_end_time').text(d.end_time.timeFormat("Y年m月d日"));
            if(d.headpath == ''){
                $('.page_raiseDetail .headimg').attr('src','http://img.yummy194.cn/20160608/99a2e57830afb6d3071939576771a023eae18b59.jpg');
            }else{
                $('.page_raiseDetail .headimg').attr('src',d.headpath);
            }
            ...
            $('.page_raiseDetail h1').text(d.title);
            $('.page_raiseDetail h2').text(d.introduction);
            $('.page_raiseDetail .dec').append(contentAppend);
            ...
            if (isEnd || isPreview) {
                $('.page_raiseDetail .status_check a.contact, .page_raiseDetail .status_check a.raiseReturn').remove();
            }

            //预告
            if (isPreview) {
                $('.page_raiseDetail .c_list').remove();
            }
            ...
            //分享绑定
            $('.page_raiseDetail .shares').click(function(){
 ...
 //               $.alert('即将开始');
            ...
            if(d.isReminder == 0 || !isCollect){
                $('.page_raiseDetail .remider').on('click',function () {
                    reminder(2, raiseDetailObject.raise_id, 0, function (d) {
                        if(d.status == 1){
                ...
                // $('.page_raiseDetail .remider').off('click');
                $('.page_raiseDetail .remider').on('click',function () {
                    ajax('Home/Index/OpenReminder',{type:2,type_id:raiseDetailObject.raise_id, d: 1},function (d) {
                        if(d.status == 1){
                            $('.page_raiseDetail .remider').html('<i></i>已开启提醒');
                            $('.page_raiseDetail .remider').css('background','#ccc');
                            $('.page_raiseDetail .remider').off('click');
            ...
            //关注按钮
            $('.page_raiseDetail .follow-btn').on('click',function () {
                var $followBtn = $('.page_raiseDetail .follow-btn');
                ...
                // setcollect(raiseDetailObject.raise_id, isCollect, function (isCollectChange) {
                //     isCollect = isCollectChange;
                //     if (isCollectChange === 1) {
                //         $followBtn.text('取消关注');
                //         $followBtn.addClass('is-collect');
                //         collectedCount += 1;
                //         $('.page_raiseDetail .collected-count').text('关注人数：' + collectedCount);
                //     } else {
                //         $followBtn.text('关注');
                ...
                var remNum = isReminder
                    ? 1
                    : 0;
                reminder(2, raiseDetailObject.raise_id, remNum, function (data) {
                    if(!member) {
                        // login 跳转回来，自动关注
                        var clickCollectBtn = function () {
                            ajax('Goods/Raise/getDetail', {'raise_id':raiseDetailObject.raise_id}, function(d) {
                                ...
                                if (d.isReminder == '1') {
                                    ...
                                    $('.page_raiseDetail .follow-btn').text('已关注');
                                    $('.page_raiseDetail .follow-btn').addClass('is-collect');
                                    return;
                                }
                                // 本来没关注
                                $('.page_raiseDetail .follow-btn').eq(0).click();
                            })
                        }
            ...
            $('.clicklogo').click(function() {
                if ($('.page_raiseDetail.wrapper').scrollTop() < $('.page_raiseDetail .c_video').height()) {
