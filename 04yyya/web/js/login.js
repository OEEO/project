var loginObject = {
    onload : function(){
        ...
        function setcode() {
            ...
            ajax('Member/Index/img', {}, function (d) {
                console.log(d);
                code = d.code;
//                $('.tpcode').attr('src','http://api.m.yami.ren/Member/Verify/verify?code=' + code);
                $('.tpcode').attr('src','http://api.yummy194.cn/Member/Verify/verify?code=' + code);
//                console.log(code);
//                console.log(d);
            });
        }  
...
/*
        $.ajax({
            url : 'http://api.yummy194.cn/Member/Verify/verify?code=' + code,
            async : false,
            type : 'GET',
...
*/
 ...
 //           $('.tpcode').attr('src','http://api.m.yami.ren/Member/Verify/verify?code=' + code);
        ...
        $('.loginBox .sendcode').click(function () {
            ...
            if (!document.dx_box.pic_code.value) {
                $.alert('图片验证码不正确', 'error');
                return;
            }
            
            ajax('Member/Index/img', {imgcode: document.dx_box.pic_code.value}, function (d) {                
                if (d.statu == 0) {
                    $.alert('图片验证码不正确', 'error');
                    return;
                } else {
                    ajax('Member/Index/sendSMS', {telephone: tel, imgcode: document.dx_box.pic_code.value}, function (d) {
                        if (d.status == 1) {
                            (function timejump(s) {
                                s--;
                            ...
                            $.alert(d.info, 'error');
                                    ...
                                    /*
            ajax('Member/Verify/verify', {code: document.dx_box.pic_code.value}, function (d) {
                if (d.check_code == 1) {
                    console.log(document.dx_box.pic_code.value);
                } else {
                    $.alert(d.info,'error');
                }
            });
